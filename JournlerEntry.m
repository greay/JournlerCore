...
#import <JournlerCore/NSAttributedString+JournlerAdditions.h>
...
- (id) initWithProperties:(NSDictionary*)aDictionary
{
	if ( self = [super initWithProperties:aDictionary] )
	{
		...
		// regenerate the date (NSInteger), cached and used extensively
...
- (void) setCollections:(NSArray*)anArray
{
	...
	{
		[collections release];
		collections = [anArray mutableCopyWithZone:[self zone]];
		// should I set it dirty?
	}
}
...
- (void) setResources:(NSArray*)anArray
{
	...
	{
		[resources release];
		resources = [anArray mutableCopyWithZone:[self zone]];
...
- (void) setLastResourceSelectionID:(NSNumber*)aNumber
{
	if ( lastResourceSelectionID != aNumber )
	{
		...
		lastResourceSelectionID = [aNumber retain];
	}
}
...
{ 
	...
	return [[_properties objectForKey:PDEntryCalDate] descriptionWithCalendarFormat:@"%Y-%m-%d" timeZone:nil locale:nil];		
}
...
- (void) deriveTitleFromContent
{
	...
	NSString *content = [[self valueForKey:@"attributedContent"] string];
	if ( content != nil && [content length] > 0 )
	{
		...
		if ( i > 0 )
		{
			...
			[self setValue:titleSubstring forKey:@"title"];
		}
	}
}
...
{
	// entry dates are used extensively
	// so the date is cached as a simple integer
	// that can be quickly accessed and compared
...
- (NSURL*) fileURLForResourceFilename:(NSString*)filename {
	...
	NSString *resourcePath = [[[self resourcesPathCreating:YES] stringByAppendingPathComponent:filename] 
...
- (JournlerResource*) resourceForABPerson:(ABPerson*)aPerson
{
	...
	// first query the journal for a resource that already encapsulates this information
	returnResource = [[self journal] alreadyExistingResourceWithType:kResourceTypeABRecord 
			data:[aPerson uniqueId] 
			operation:kNewResourceForceLink];
...
- (JournlerResource*) addResource:(JournlerResource*)aResource
{
	...
	if ( resourceIndex == NSNotFound )
	{
		...
		if ( ![[aResource valueForKey:@"entries"] containsObject:self] ) 
			[[aResource mutableArrayValueForKey:@"entries"] addObject:self];
		...
		// dummy number method
		//[self setNumberOfResources:[entryResources count]]; -- why is this necessary?
		//	kvo on the entriestable? -- replace with .resources.@count maybe?
		//#warning can I do this with kvo notifications and remove this method?
		//[self setNumberOfResources:[self countOfResources]];
	...
	{
		returnResource = [self objectInResourcesAtIndex:resourceIndex];
	}
...
- (BOOL) removeResource:(JournlerResource*)aResource
{
	...
	if ( resourceIndex != NSNotFound )
	{
		...
		#warning why am I not removing self from the resource's entries array?
...
- (NSString*) pathToPackage 
{
	...
	completePath = [[[self journal] pathForSupportDocumentOrDirectory:JournlerEntriesDirectory] stringByAppendingPathComponent:entryPath];
...
+ (BOOL) canImportFile:(NSString*)fullpath {
	...
	else if ( UTTypeConformsTo((CFStringRef)import_uti,(CFStringRef)kUTTypeJournlerMailEmailExtended) )
		can_import = YES;
	
	else if ( UTTypeConformsTo((CFStringRef)import_uti,(CFStringRef)kUTTypeJournlerMailEmail) )
		can_import = YES;
...
- (id) initWithImportAtPath:(NSString*)fullpath 
	...
	else if ( UTTypeConformsTo((CFStringRef)import_uti,kUTTypeRTFD) )
	{
		...
		{
			import_content = [[[NSAttributedString allocWithZone:[self zone]]
					initWithRTFDFileWrapper:import_wrapper 
					documentAttributes:&docAttributes] autorelease];
		}
		else
		{
	...
	else if ( [[fullpath pathExtension] isEqualToString:kTextClippingExtension] ) 
	{
		...
		else
		{
			if ( [textClipping isRichText] )
				...
				import_content = [[[NSAttributedString alloc] 
						initWithString:[textClipping plainTextRepresentation] 
						attributes:[JournlerEntry defaultTextAttributes]] autorelease];
		}
	}
	...
	_importModificationDate = [[[[NSFileManager defaultManager] 
			fileAttributesAtPath:fullpath traverseLink:YES] 
			objectForKey:NSFileModificationDate] retain];
...
{	
	...
	if ( _import_path != nil ) 
	{
		...
		if ( [[NSWorkspace sharedWorkspace] canWatchFile:_import_path] || [[NSWorkspace sharedWorkspace] canPlayFile:_import_path]
				...
				|| [[NSWorkspace sharedWorkspace] file:_import_path conformsToUTI:kUTTypeJournlerMailEmailExtended] 
				|| [[NSWorkspace sharedWorkspace] file:_import_path conformsToUTI:kUTTypeJournlerMailEmail] )
		{
...
- (NSAttributedString*) prepWithTitle:(BOOL)wTitle category:(BOOL)wCategory smallDate:(BOOL)wDate {
	...
	if ( textualContent ) 
	{
		// appaned a space between the header and content
...
- (BOOL) _writeiPodNote:(NSString*)contents iPod:(NSString*)path 
{
	...
	[mutable_string replaceOccurrencesOfString:[NSString stringWithCharacters:(const unichar[]) {NSAttachmentCharacter} length:1] 
			...
			range:NSMakeRange(0, [mutable_string length])];
	...
	else 
	{
		@try
		{
			...
			while ( [workingContents length] > 0 ) {
				...
				if ( ![thisFilesContents writeToFile:[path stringByAppendingPathComponent:thisFilesTitle] 
						atomically:YES 
						encoding:NSUnicodeStringEncoding 
						error:&error] )
				{
					NSLog(@"Unable to export entry %@ to path %@, error: %@", [self tagID], 
							[path stringByAppendingPathComponent:thisFilesTitle], error );
					return NO;
				}
...
{
	NSString *defaultCategory = [[NSUserDefaults standardUserDefaults] stringForKey:@"JournlerDefaultCategory"];
...
{
	NSString *defaultCategory = [[NSUserDefaults standardUserDefaults] stringForKey:@"DropBoxCategory"];
	if ( [defaultCategory isEqualToString:@"-"] ) return nil;
	else return defaultCategory;
}
...
- (NSString*) htmlString
{
	...
	if ( [[NSUserDefaults standardUserDefaults] boolForKey:@"ExportsUseAdvancedHTMLGeneration"] )
		strippedHTML = [strippedContent attributedStringAsHTML:kUseSystemHTMLConversion|kUseInlineStyleDefinitions|kConvertSmartQuotesToRegularQuotes 
			documentAttributes:nil 
			avoidStyleAttributes:[[NSUserDefaults standardUserDefaults] stringForKey:@"ExportsNoAttributeList"]];
	else
		strippedHTML = [strippedContent attributedStringAsHTML:kUseJournlerHTMLConversion|kConvertSmartQuotesToRegularQuotes 
				documentAttributes:nil 
				avoidStyleAttributes:nil];
