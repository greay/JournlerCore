...
- (NSAttributedString*) attributedStringWithoutTextAttachments
{
	...
	{
		id attachment = [attributedString attribute:NSAttachmentAttributeName 
				atIndex:i 
				effectiveRange:nil];
...
{	
	NSMutableAttributedString *mutableString = [[self mutableCopyWithZone:[self zone]] autorelease];
	
	NSRange limitRange;
	NSRange effectiveRange;
	id attr_value;
	 
	limitRange = NSMakeRange(0, [mutableString length]);
	 
	while (limitRange.length > 0) {
		
		attr_value = [mutableString attribute:NSLinkAttributeName 
...
{	
	NSMutableAttributedString *mutable_str = [[self mutableCopyWithZone:[self zone]] autorelease];
	...
	while (limitRange.length > 0) {
		...
		attr_value = [mutable_str attribute:NSLinkAttributeName 
				atIndex:limitRange.location 
				longestEffectiveRange:&effectiveRange 
				inRange:limitRange];
		...
		if ( [attr_value isKindOfClass:[NSURL class]] && [attr_value isJournlerEntry] ) 
		{
			...
			JournlerEntry *anEntry = [aJournal entryForTagID:[NSNumber numberWithInt:[[[attr_value absoluteString] lastPathComponent] intValue]]];
			if ( anEntry != nil ) {
...
- (NSData*) firstImageData:(NSRange)aRange fileType:(NSBitmapImageFileType)type	
{
	...
	while (limitRange.length > 0) 
	{
		attr_value = [self attribute:NSAttachmentAttributeName 
				atIndex:limitRange.location 
				longestEffectiveRange:&effectiveRange 
				inRange:limitRange];
...
bail:
	...
	if ( options & kConvertSmartQuotesToRegularQuotes )
	{
		...
		[workingString replaceOccurrencesOfString:openSmartQuote 
				withString:@"\"" 
				options:NSLiteralSearch 
				range:NSMakeRange(0,[workingString length])];
		[workingString replaceOccurrencesOfString:closeSmartQuote 
				withString:@"\"" 
				options:NSLiteralSearch 
				range:NSMakeRange(0,[workingString length])];
...
{	
	...
	for ( i = 0; i < [self length]; i++ ) {
				...
				[[self string] characterAtIndex:i] == NSCarriageReturnCharacter ) 
		{
			...
			{
				if ( i != 0 && ( [[self string] characterAtIndex:i-1] == NSNewlineCharacter 
						|| [[self string] characterAtIndex:i-1] == NSCarriageReturnCharacter ) )
				{
				...
				else
				{
					...
					if ( [[self string] characterAtIndex:i+1] == NSNewlineCharacter 
							|| [[self string] characterAtIndex:i+1] == NSCarriageReturnCharacter )
			...
			if ( linkURL && [linkURL isKindOfClass:[NSURL class]] ) {
				...
				if ( [linkURL isJournlerEntry] || [linkURL isJournlerResource] ) {
					...
					if ( [[self string] characterAtIndex:i] == NSAttachmentCharacter )
						...
						blue_text = (NSString*)CFURLCreateStringByReplacingPercentEscapesUsingEncoding(NULL,
								(CFStringRef)linkSub,
								CFSTR(""),
					...
					linkString = [NSString stringWithFormat:@"<a href=\"%@\">%@</a>", [linkURL absoluteString],
						(NSString*)CFURLCreateStringByReplacingPercentEscapesUsingEncoding(NULL,
								(CFStringRef)linkSub,
								CFSTR(""),
								kCFStringEncodingUTF8)];
...
{	
	...
	while ( aStyleDefinition = [stylesEnumerator nextObject] )
	{
		...
		if ( bannedAttributesList != nil && [bannedAttributesList count] != 0 )
		{
			...
			for ( i = 0; i < [bannedAttributesList count]; i++ )
			{
				...
				while ( YES )
				{
					...
					if ( [aStyleDeclaration rangeOfString:bannedAttribute 
							options:NSCaseInsensitiveSearch 
							range:NSMakeRange(0,[aStyleDeclaration length])].location == 0 )
						startIndex = 0;
					else
					{
		...
		[htmlInline replaceOccurrencesOfString:classProperty 
				withString:styleProperty 
				options:NSLiteralSearch 
				range:NSMakeRange(0,[htmlInline length])];
	}
	
	//NSLog([stylesDictionary description]);
	
	// do some final clean up 
	[htmlInline replaceOccurrencesOfString:zeroMargin 
			withString:@"" 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
	
	[htmlInline replaceOccurrencesOfString:emptyStyle 
			withString:@"" 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
			
	[htmlInline replaceOccurrencesOfString:emptyParagraph 
			withString:@"" 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
	
	[htmlInline replaceOccurrencesOfString:boldOpen 
			withString:strongOpen 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
			
	[htmlInline replaceOccurrencesOfString:boldClose 
			withString:strongClose 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
	
	[htmlInline replaceOccurrencesOfString:italicOpen 
			withString:emphasisOpen 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
			
	[htmlInline replaceOccurrencesOfString:italicClose 
			withString:emphasisClose 
			options:NSLiteralSearch 
			range:NSMakeRange(0,[htmlInline length])];
