...
#import <JournlerCore/JournlerKQueue.h>
#import <JournlerCore/JournlerMainThreadProxy.h>
#import <unistd.h>
#import <fcntl.h>
...
static JournlerKQueue * gUKKQueueSharedQueueSingleton = nil;


@implementation JournlerKQueue

// Deprecated:
#if UKKQUEUE_OLD_SINGLETON_ACCESSOR_NAME
+(JournlerKQueue*) sharedQueue
{
...
+(id) sharedFileWatcher
{
    AT_SYNCHRONIZED( self )
    {
        if( !gUKKQueueSharedQueueSingleton )
            gUKKQueueSharedQueueSingleton = [[JournlerKQueue alloc] init];	// This is a singleton, and thus an intentional "leak".
    }
...
-(void)		watcherThread: (id)sender
{
    ...
    while( keepThreadRunning )
    {
			...
			if( n > 0 )
			{
				if( ev.filter == EVFILT_VNODE )
				{
					if( ev.fflags )
					{
						...
						//NSLog(@"JournlerKQueue: Detected file change: %@", fpath);
		...
		NS_HANDLER
			NSLog(@"Error in JournlerKQueue watcherThread: %@",localException);
		NS_ENDHANDLER
