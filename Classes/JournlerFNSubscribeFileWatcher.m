...
#import <JournlerCore/JournlerFNSubscribeFileWatcher.h>
...
@implementation JournlerFNSubscribeFileWatcher
...
{
	static JournlerFNSubscribeFileWatcher* sSharedFileWatcher = nil;
	
	if( !sSharedFileWatcher )
		sSharedFileWatcher = [[JournlerFNSubscribeFileWatcher alloc] init];	// This is a singleton, and thus an intentional "leak".
...
-(void) addPath: (NSString*)path
{
    ...
    if( err != noErr )
    {
        NSLog( @"JournlerFNSubscribeFileWatcher addPath: %@ failed due to error ID=%ld.", path, err );
        return;
    }
...
-(void) sendDelegateMessage: (FNMessage)message forSubscription: (FNSubscriptionRef)subscription
{
    ...
    //NSLog( @"JournlerFNSubscribeFileWatcher noticed change to %@", path );	// DEBUG ONLY!
}
...
void    UKFileSubscriptionProc( FNMessage message, OptionBits flags, void *refcon, FNSubscriptionRef subscription )
{
    JournlerFNSubscribeFileWatcher*   obj = (JournlerFNSubscribeFileWatcher*) refcon;
